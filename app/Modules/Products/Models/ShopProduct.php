...
use App\Models\ShopProductNotAvailableIn;
use App\Models\ShopProductPromotion;
use App\Modules\Common\Models\ShopCategory;
use App\Modules\Orders\Models\ShopOrderDetail;
...
class ShopProduct extends MyBaseApiModel
{
 ...
 */
    ...
    public function extras() {
        return $this->belongsToMany(ShopExtraProduct::class, 'shop_product_shop_extraproduct', 'product_id', 'extraproduct_id');
    }
    ...
    public function not_available_in() {
        return $this->hasMany(ShopProductNotAvailableIn::class, 'product_id', 'id');
    }
    ...
    public function images_portrait() {
        return $this->hasMany(ShopProductImage::class, 'product_id', 'id')->where('type', 'portrait');
    }

    public function images_landscape() {
        return $this->hasMany(ShopProductImage::class, 'product_id', 'id')->where('type', 'landscape');
    }

    public function productDescription() {
        return $this->hasOne(ShopProductDescription::class, 'product_id', 'id')->where('lang', $this->lang);
    }

    public function productDescriptionEnglish() {
        return $this->hasOne(ShopProductDescription::class, 'product_id', 'id')
                ->where('lang', 'en')
                ->select(['product_id', 'name', 'description', 'content', 'ingredients', 'allergies', 'gluten_free', 'weight']);
    }
    ...
    public function details() {
        ...
        return $this->hasOne(ShopProductDescription::class, 'product_id', 'id')->where('lang', $lang);
    }
    ...
    public function sizes() {
        return $this->hasMany(ShopProductAttribute::class, 'product_id', 'id')->select('id', 'name', 'price')->where('attribute_group_id', 1)->orderBy('sort', 'asc');
    }
...
// Not SC_PRODUCT_GROUP
    ...
    /*
    * create join with prmotions based on condition
    * return valid promotions
    */
    public function validPromotions()
    {
      return $this->hasOne(ShopProductPromotion::class, 'product_id', 'id')
                  ->where('status_promotion', 1)
                  ->where(
                      function ($query) {
                          $query->where('shop_product_promotion.date_end', '>=', date("Y-m-d"))
                                ->orWhereNull('shop_product_promotion.date_end');
                      }
                  )
                  ->where(
                      function ($query) {
                          $query->where('shop_product_promotion.date_start', '<=', date("Y-m-d"))
                                ->orWhereNull('shop_product_promotion.date_start');
                      }
                  );
    }//validPromotions
