...
class ShopCart extends GeneralController
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $product = ShopProduct::find($item_id);
        ...
        if ($product->allowSale()) {
            ...
            $price=($product->sizes->count())?$product->getFinalPrice():$product->getFinalPrice();
            ...
            return new JsonResponse([
                'status'=>0,
                'message' => 'This Item is not available for sale right now',
                'data' => [],
            ],200 );
    ...
    public function addToCart_ajax(Request $request)
    {
        ...
        if ($product->attributes->count() || $product->kind == SC_PRODUCT_GROUP) {
            ...
            /* if the product have size u need validate to select size */
