...
class ShopCustomerController extends Controller {
    ...
    public function index() {
        $status = request('type', 0);
        $role = request('user', NULL);
        $title = '';
        if ($role == NULL && $status == 0) {
            $title = trans('customer.admin.list');
        } elseif ($status == 1) {
            $title = trans('customer.guest');
        } else {
            $title = trans('customer.user');
        }
        $data = [
            'title' => $title,
            'sub_title' => '',
        ...
        $obj = $obj->where('is_guest', $status);
        //if($role!=null){
        $obj = $obj->where('role', $role);
        /* }else{
          $obj=$obj->where('role',NULL);
          } */
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[] = [
                ...
                'action' => '
                    ...
                    <a href="' . route('admin_customer.details', ['id' => $row['id']]) . '"><span title="' . trans('customer.admin.details') . '" type="button" class="btn btn-flat btn-primary"><i class="fa fa-eye"></i></span></a>&nbsp;
    ...
    public function details($id){
        ...
        $data = [
            'title' => trans('customer_address.admin.title'),
            'sub_title' => '',
            'icon' => 'fa fa-indent',
            'url_delete_item' => '',
        ];
        ...
        $data['listTh'] = [
            'id' => trans('customer_address.id'),
            'tag' => trans('customer_address.tag'),
            'address' => trans('customer_address.address'),
            'landmark' => trans('customer_address.landmark'),
            'emirate' => trans('customer_address.emirate'),
            'default' => trans('customer_address.default')
        ];

        $addresses = ShopUserAddress::where('user_id', $id)->get();
        $dataTr = [];
        foreach ($addresses as $key => $address) {
            $dataTr[] = [
                ...
                'default' => $address['default'] ? '<span class="label label-success">Yes</span>' : '<span class="label label-danger">No</span>',
            ];
        }
