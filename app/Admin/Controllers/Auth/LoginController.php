...
class LoginController extends Controller {
    ...
    public function postLogin(Request $request) {
        ...
        //  $_SESSION['roles_meny_array'] = ;
        if ($this->guard()->attempt($credentials, $remember)) {
            
            //rules for using menu
            $role_id = Admin::User()->roles[0]->id; //rule id
            $roles_menu = \App\Admin\Models\AdminMenuRole::select('menu_id')->where('role_id', $role_id)->get(); //role menus
            $roles_meny_array = [];
            foreach ($roles_menu as $rol) {
                $roles_meny_array[] = $rol->menu_id;
            }
            
            session(['roles_meny_array' => $roles_meny_array]);
            //end rules
...
//Edit
        if (!empty($data['avatar'])) {
            $data['avatar'] = str_replace(env('APP_URL') . '/', '', $data['avatar']);
        }
        $dataUpdate = [
            'name' => $data['name'],
            'avatar' => $data['avatar'],
