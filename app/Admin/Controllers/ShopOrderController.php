...
{
    public $statusPayment, $statusOrder, $statusShipping, $statusOrderMap, $statusShippingMap, $currency, $country, $countryMap, $brand, $branch, $typeOrder;
    ...
    public function index()
    {
        ...
        $listTh = [
            ...
            'number' => trans('order.admin.number'),
            ...
            'payment_status' => trans('order.admin.payment_status'),
        ...
        $order_brand = request('order_brand') ?? '';
        $order_branch = request('order_branch') ?? '';
        ...
        $obj = new ShopOrder;
        if ((int) $order_brand) {
            $obj = $obj->where('brand_id', (int) $order_brand);
        }
        if ((int) $order_branch) {
            $obj = $obj->where('store_id', (int) $order_branch);
        }
        if ($order_type) {
            $obj = $obj->where('order_type',  $order_type);
        }
        if ((int) $order_status) {
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $total = $row['total'] + $row['shipping'];
            $dataTr[] = [
                'check_row' => '<input type="checkbox" class="grid-row-checkbox" data-id="' . $row['id'] . '">',
                'id' => $row['id'],
                'number' => $row['number'],
                'email' => $row['email'] ?? 'N/A',
                'subtotal' => sc_currency_render_symbol(calculateOrdertotal($row) ?? 0, $row['currency']),
                ...
                'total' => sc_currency_render_symbol($total ?? 0, $row['currency']),
                'payment_method' => strtoupper($row['transaction_type']),
                'payment_status' => orderPaymentStatus($row),
      ...
      $.pjax({url: url, container: '#pjax-container'})
        ...
        foreach ($this->brand as $key => $brand) {

            $optionBrand .= '<option  ' . (($order_brand == $key) ? "selected" : "") . ' value="' . $key . '">' . $brand . '</option>';
        }
        foreach ($this->branch as $key => $branch) {

            $optionBranch .= '<option  ' . (($order_branch == $key) ? "selected" : "") . ' value="' . $key . '">' . $branch . '</option>';
        }
        foreach ($this->typeOrder as $key => $type) {
            $optionType .= '<option  ' . (($order_type == $key) ? "selected" : "") . ' value="' . $key . '">' . $type . '</option>';
        }
        $data['menu_search'] = '
                <form action="' . route('admin_order.index') . '" id="button_search">
                   ...
                   <div class="btn-group pull-right"  style="margin-right: 10px">
                    ...
                    <div class="btn-group pull-right"  style="margin-right: 10px">
                        <div class="form-group">
                           <select class="form-control" name="order_branch">
        ...
        /* <div class="btn-group pull-right"  style="margin-right: 10px">
                        <div class="form-group">
                           <select class="form-control " name="order_brand">
    ...
    public function detail($id)
    {
        ...
        //dd($order->store->description->title);
        $rating = (array) OrderRatingService::getOrderRatingAndReview($id);
        //dd($rating);
        if ($order === null) {
            return 'no data';
        }
