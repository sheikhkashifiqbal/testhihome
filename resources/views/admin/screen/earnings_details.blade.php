...
<div class="box">
    ...
    <div class="row">
        <div class="col-sm-4">
            <img style="width: 300px;" src="{{url('/').'/'.$order->store['logo']}}"/>
        </div>
        <div class="col-sm-8">
            <h4>{{$order->store->description['title']}}</h4>
            <h5>{{trans('order.admin.number').' : '.$order['number']}}</h5>
            <h5>{{trans('order.shipping_address').':'.$order->store->description['address']}}</h5>
            <h5>{{$order->store['time_active']}}</h5>
            <h5>{{trans('order.admin.total').' : '.$order['total']}} AED</h5>
            <h5>{{trans('order.seller_earning').' : '.calculateSellerEarning($order['total'])}} AED</h5>
            <h5>{{trans('order.hihome_earning').' : '.calculateHiHomeCommision($order['total'])}} AED</h5>
        </div>
    </div>
</div>
</div>
