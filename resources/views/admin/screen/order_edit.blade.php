          ...
          <div class="row" id="order-body">
            <div class="col-sm-6">
                 <table class="table table-bordered">
                    <tr>
                      <td class="td-title">{{ trans('order.admin.number') }}:</td>
                      <td>{{$order->number}}</td>
                    </tr>
                    <tr>
                ...
                <table class="table table-bordered">
                   <tr>
                     <td class="td-title">{{ trans('order.special_request') }}:</td><td>{{ $order->special_request }}</td>
                   </tr>
               </table>
            </div>
            <div class="col-sm-6">
                <table  class="table table-bordered">
                    ...
                    <tr><td>{{ trans('order.seller_name') }}:</td><td>{{ $store->description->title ??'' }}</td></tr>
                    <tr><td>{{ trans('order.seller_phone') }}:</td><td>{{ $store->legal_business_phone ??'' }}</td></tr>
                    <tr><td>{{ trans('order.seller_email') }}:</td><td>{{ $store->legal_business_email ??'' }}</td></tr>
                    <tr><td>{{ trans('order.delivery_date') }}:</td><td>{{ ($order->delivery_date) ? \Carbon\Carbon::parse($order->delivery_date)->format('d-m-Y')." @ " : '' }}  {{ $order->delivery_slot }}</td></tr>
                    <tr><td>{{ trans('order.pickup_time') }}:</td><td> {{ ($order->pickup_time) ? \Carbon\Carbon::parse($order->pickup_time)->format('d-m-Y h:i a') : '' }}</td></tr>
                    <tr><td>{{ trans('order.customer_loc') }}:</td><td><a href="https://www.google.com/maps/place/{{ $order->address1_lat }},{{ $order->address1_long }}" target="_blank">https://www.google.com/maps/place/{{ $order->address1_lat }},{{ $order->address1_long }} </td></tr>
                    <tr><td>{{ trans('order.seller_loc') }}:</td><td><a href="https://www.google.com/maps/place/{{ $store->lat }},{{ $store->long }}" target="_blank"> https://www.google.com/maps/place/{{ $store->lat }},{{ $store->long }} </td></tr>
                    <tr>
                        <td>{{ trans('order.admin.payment_method_short') }}:</td>
                        <td>{{ $order->transaction_type ??'' }} [{{ $order->transaction ??'' }}]</td>
                    </tr>
                    <tr>
                        <td>{{ trans('order.admin.payment_status') }}:</td>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    @endforeach
                          ...
                          <tr>
                            <td colspan="4" class="product_price">{{ trans('order.sub_total') }}</td>
                            <td class="product_price">{{ sc_currency_render_symbol(calculateOrdertotal($order), $order->currency) }}</td>
                            <td>&nbsp;</td>
                          </tr>
                          <tr>
                            <td colspan="4" class="product_price">{{ trans('order.discount') }}</td>
                            <td class="product_price">{{ sc_currency_render_symbol($order->discount, $order->currency) }}</td>
                            <td>
                              @if($order->discount)
                                [ {{$order->offer_code}} ]
                              @endif
                          ...
                          <tr>
                            <td colspan="4" class="product_price">{{ trans('order.deliver_price') }}</td>
                            <td class="product_price">{{ sc_currency_render_symbol($order->shipping, $order->currency) }}</td>
                            <td>&nbsp;</td>
                          </tr>
                          <tr>
                            <td colspan="4" class="product_price">{{ trans('order.total') }}</td>
                            <td class="product_price">{{ sc_currency_render_symbol($order->total + $order->shipping, $order->currency) }}</td>
                            <td>&nbsp;</td>
                          </tr>
                    </form>
                </tbody>
              ...
              <div class="box collapsed-box">
                  <table class="table table-bordered">
                      <tr style="background:#f5f3f3;font-weight: bold;">
                        <td colspan="2">{{ trans('rating.admin.rating_review') }}</td>
                      </tr>
                      @if(!empty($rating))
                        <tr>
                            <td>{{ trans('rating.admin.review') }}:</td>
                            <td style="text-align:right">{{$rating['review'] }}</td>
                        </tr>
                        @foreach ($rating['products'] as $product)
                          @php $product = $product->toArray([]) @endphp
                          <tr>
